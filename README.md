...
- **`GET /api/bookmarks`**: ブックマークのリストを取得します。このエンドポイントは、オプションページでの API 接続確認と、拡張機能アイコンの状態を更新するために使用されます。ブックマークオブジェクトの配列（例: `[{ "bookmark_id": 1, "url": "https://example.com", "title": "Example" }]`）を JSON 形式で返す必要があります。
