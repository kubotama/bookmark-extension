...
- **`useApiUrl`**: API のベース URL の取得・設定と、ブックマーク追加・取得用 URL の生成ロジックをカプセル化します。`chrome.storage`から不正なデータ（文字列以外など）が読み込まれた場合でも、デフォルトURLにフォールバックすることで、実行時エラーを防ぎアプリケーションの安定性を高めています。
