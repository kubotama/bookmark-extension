...
- **カスタム API エンドポイント**: オプションページで、ブックマーク登録先の API エンドポイントを自由に設定できます。URLが未入力の場合や、`http://` または `https://` で始まらない無効な形式のURLが入力された場合にはエラーメッセージが表示され、設定が保存されないように検証機能が強化されています。
